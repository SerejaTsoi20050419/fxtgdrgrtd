...
const initSwipers = () => {
  ...
  const textSpan = toggleDevicesBtn.querySelector(".more__text");

  toggleDevicesBtn.addEventListener("click", function () {
    moreDevices.classList.toggle("more__content--shrinked");
    toggleDevicesBtn.classList.toggle("more__button--expanded");

    const expanded = moreDevices.classList.contains("more__content--shrinked");
    textSpan.textContent = expanded ? "Показать все" : "Скрыть";
  });
