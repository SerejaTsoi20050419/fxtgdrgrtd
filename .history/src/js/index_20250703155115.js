...
closeBtn.addEventListener('click', () => {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    const hiddenItems = document.querySelectorAll('.selector__item.hidden');
    const moreText = document.querySelector('.more__text');
    const moreImage = document.querySelector('.more__image');
    ...
    moreButton.addEventListener('click', function () {
      ...
      // Обновляем swiper, чтобы он пересчитал ширину
      swiper.update();

      expanded = !expanded;
      moreText.textContent = expanded ? 'Скрыть' : 'Показать все';
      moreImage.classList.toggle('rotated');
    });
  });
