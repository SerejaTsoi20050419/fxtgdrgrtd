...
const initSwipers = () => {
  ...
  toggleBtn.addEventListener('click', () => {
   ...
   document.addEventListener("DOMContentLoaded", function () {
    ...
    const toggleIcon = toggleButton.querySelector(".more__image");

    toggleButton.addEventListener("click", function () {
      swiperContainer.classList.toggle("more__content--shrinked");

      const isExpanded = !swiperContainer.classList.contains("more__content--shrinked");

      toggleText.textContent = isExpanded ? "Скрыть" : "Показать все";

      toggleIcon.classList.toggle("more__image--expand");
      toggleIcon.classList.toggle("more__image--collapse");
    });
  });
