...
function initDeviceSwiper() {
  if (window.innerWidth < 768 && !deviceSwiper) {
    deviceSwiper = new Swiper('#more-devices', { ... });
  } else if (window.innerWidth >= 768 && deviceSwiper) {
    deviceSwiper.destroy(true, true);
    deviceSwiper = null;
  }
}

window.addEventListener('resize', initDeviceSwiper);
initDeviceSwiper();
