...
closeBtn.addEventListener('click', () => {
  ...
  document.getElementById('toggleBrands').addEventListener('click', function () {
    ...
    const swiper = new Swiper('#more-brands', {
      slidesPerView: 'auto',
      spaceBetween: 16,
      pagination: {
...
const moreText = button.querySelector('.more__text');

button.addEventListener('click', () => {
  if (hiddenSlides[0].classList.contains('hidden')) {
  ...
  // Если используешь Swiper, обнови его:
  if (window.swiperBrands) {
    window.swiperBrands.update();
  }
});
document.querySelector('.more__button').addEventListener('click', function() {
  // Получаем все скрытые слайды
  const hiddenSlides = document.querySelectorAll('.swiper-slide.hidden');
  
  // Показываем их, убирая класс hidden
  hiddenSlides.forEach(slide => slide.classList.remove('hidden'));
  
  // Можно изменить текст кнопки, если нужно
  this.querySelector('.more__text').textContent = 'Скрыть';
  
  // Чтобы кнопка могла переключать видимость, можно добавить логику:
  // Например, если уже открыты, скрыть обратно
});
  });
