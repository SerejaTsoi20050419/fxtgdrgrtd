...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('toggleDevices').addEventListener('click', function () {
    const deviceBlock = document.getElementById('more-devices');
    const expandText = document.getElementById('expandText');

    // Переключаем класс, который ограничивает высоту
    deviceBlock.classList.toggle('more__content--shrinked');

    // Меняем текст кнопки
    if (deviceBlock.classList.contains('more__content--shrinked')) {
      expandText.textContent = 'Показать все';
    } else {
      expandText.textContent = 'Скрыть';
    }
